...
void Chat::showMainMenu()
{
	...
	menuChoice = _getch(); // unbuffered single char input
...
bool Chat::registerNewUser()
{
	...
	try { 
		if (!registeredUsersMap_.empty() && (registeredUsersMap_.find(login) != registeredUsersMap_.end() || login == "all")) // std::map::contains() works since C ++ 20
		{
		...
		std::cin >> password;
		std::cout << "-------------------------" << std::endl;
		currentUser_ = std::make_shared<User>(User(login, password, name)); // makes currentUser_ = new user
		registeredUsersMap_[login] = currentUser_; // and adds new user to the map
		return true;
	}
...
{	
	...
	if (registeredUsersMap_.find(login) == registeredUsersMap_.end() || // login is not registered           // std::map::contains() won't work before C++20
		!(registeredUsersMap_[login]->checkPassword(password))) // or password is incorrect
	{
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), RED);
		std::cout << "Wrong login / password!" << std::endl;
		return false;
	}
	currentUser_ = registeredUsersMap_[login]; // if login / password are OK -> set currentUser
	return true;
}

const void Chat::showAllRegisteredUsers() const
{
	std::cout << "-------------------------" << std::endl;
